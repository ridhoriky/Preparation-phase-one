...
function minDistanceBetweenGreatest(arr) {
  // your code here
  let arrIndex = [];
  let elementSebelum = 0;
  for (const index in arr) {
    if (arr[index] > elementSebelum) {
      elementSebelum = arr[index];
      arrIndex = [];
      arrIndex.push(index);
    } else if (arr[index] === elementSebelum) {
      arrIndex.push(index);
    }
  }
  if (arrIndex.length <= 1) {
    ...
    for (let index = 0; index < arrIndex.length; index++) {
      let jarakDenganSebelum = arrIndex[index] - arrIndex[index - 1];
      if (index !== 0 && jarakDenganSebelum < selisihTerdekat) {
        selisihTerdekat = jarakDenganSebelum;
      }
    }
...
function averageLengthWord(words) {
  ...
  let jumlah = 1;
  for (const element of arrKata) {
    jumlah += element.length;
  }
  const rataRata = jumlah / arrKata.length;

  const result = [];
  for (const element of arrKata) {
    if (element.length === Math.round(rataRata)) {
      result.push(element);
    }
  }
  return result;
