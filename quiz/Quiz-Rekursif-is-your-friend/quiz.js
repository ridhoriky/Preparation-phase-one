...
function changeXRecursive(data, jenis) {
  ...
  let val = data[0];
  if (
    (Number(data[0]) % 2 === 0 && jenis === "ganjil") ||
    (Number(data[0]) % 2 === 1 && jenis === "genap")
  ) {
    val = "x";
  }
  let kataSisa = data.slice(1);

  return val + changeXRecursive(kataSisa, jenis);
}
...
function consonantCounterRecursive(sentences) {
  ...
  let isKonsonan = true;
  for (const val of bukanKonsonan) {
    if (sentences[0].toLowerCase() === val) {
      isKonsonan = false;
    }
  }

  const newSentence = sentences.slice(1);
  let hasil = isKonsonan ? 1 : 0;
  return (hasil += consonantCounterRecursive(newSentence));
}
...
function dividableRecursive(array, num) {
  ...
  return [...result, ...dividableRecursive(array, num)];
}
...
function hapusSimbolRec(str) {
  ...
  return (hasil += hapusSimbolRec(newStr));
}
...
function palindromeRecursive(sentence) {
  ...
  function palindrome(sentence) {
    ...
    const hurufSisa = sentence.substring(0, sentence.length - 1);

    return (sentence[sentence.length - 1] += palindrome(hurufSisa));
  }

  const kataBalik = palindrome(sentence);
  return kataBalik === sentence;
...
function parseNumber(equation) {
  ...
  const angkaString = equation.toString();
  const digitPertama = Number(angkaString[0]);
  const panjang = angkaString.length;
  const angkaSisa = Number(angkaString.substring(1));
  const nilaiDigitPertama = digitPertama * Math.pow(10, panjang - 1);

  if (angkaSisa === 0) {
    return `${nilaiDigitPertama}`;
  } else {
    return `${nilaiDigitPertama} + ` + parseNumber(angkaSisa);
  }
}
...
function virusCheckRecursive(str, viruses) {
  ...
  if (viruses) {
    arrVirus = viruses.split("|");
  }

  let hasil = 0;
  for (const target of arrVirus) {
    if (str[0].toLowerCase() === target.toLowerCase()) {
      hasil = 1;
    }
  }
  ...
  return hasil + virusCheckRecursive(newStr, viruses);
}
