  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = data[i].plat.length - 1; j > 0; j--) {
      if (Number(data[i].plat[j])) {
        angkatTerakhir = Number(data[i].plat[j]);
        break;
      }
    }
    if (data[i].type === "Mobil" && angkatTerakhir % 2 !== date % 2) {
      hasil++;
    }
  }
  return hasil;
...
function economyChangeSummary(tradeActivity) {
  ...
  function hitung(a, b, operator) {
    ...
    return operasi[operator]?.(a, c);
  }
  const result = [];
  for (const arr1 of tradeActivity) {
    for (const val of arr1) {
      const [angka, operator, nama] = getValue(val);
      deposit[nama].uang = hitung(deposit[nama].uang, angka, operator);
      result.push({
        name: nama,
        deposit: deposit[nama].uang,
        owner: deposit[nama].owner,
      });
    }
  }
  return result;
...
function initialObjectGrouping(studentsArr) {
  //CODE HERE
  const result = {};
  for (const element of studentsArr) {
...
function travelingIndonesia(arr, emoney) {
  //code here
  const listKota = ["Yogyakarta", "Semarang", "Surabaya", "Denpasar"];
  const listDiskon = {
    OVO: 0.15,
    Dana: 0.1,
    Gopay: 0.05,
  ...
  for (const element of arr) {
    const [name, departureCity, destinationCity, transport] = getData(element);
    const jarak = Math.abs(
      listKota.indexOf(departureCity) - listKota.indexOf(destinationCity)
    );
    const biaya = listBiaya[transport] * jarak;
    const diskon = biaya * listDiskon[emoney];
    hasil.push({
      name,
      departureCity,
      destinationCity,
      transport,
      totalCost: biaya - diskon,
    });
  }

  function sortHasil(hasil) {
    const arr = [...hasil];
